...
# 如果 noise 本身录音电平极低，
# 算出来需要放大 100 倍以上，则认为 noise 文件异常。
#
# 注意：不应该 clamp scale，
# 因为 clamp 后就不再满足目标 SNR。
...
# 推荐保留。
#
# clean reference 会使用和所有 noisy condition
# 完全相同的 common gain。
#
# 因此可公平比较：
#
...
def read_wav_mono(
    ...
    if len(data) == 0:
        raise ValueError(
            f"Audio empty after resampling: {path}"
        )
...
def make_deterministic_rng(
    ...
    key = (
        f"{RANDOM_SEED}|"
        f"{relative_clean_path.as_posix()}|"
        f"{noise_type}|"
        f"{realization_id}"
    )
...
def discover_noise_groups(
    noise_root,
):
    """
    推荐：
        NOISE_DIR/subway/*.wav
        NOISE_DIR/road/*.wav

    第一层子目录作为 noise type。

    如果 WAV 直接放在 NOISE_DIR 根目录，
    则归到 "default"。
    """

    all_files = sorted(
        noise_root.rglob("*.wav")
    )
...
def build_noise_track_once(
    ...
    clean <= 5 秒：
        从某一条约 5 秒 noise 中随机截取。

    clean > 5 秒：
        连续选择同类别的多个 noise 文件拼接。

    注意：
    对 KWS benchmark，
    这里不做 FG event active-power scaling，
    因为 subway / road 属于持续环境噪声。
    """

    if target_len <= 0:
        raise ValueError(
            "target_len must be > 0"
        )

    if not noise_files:
        raise ValueError(
            "Empty noise file list"
        )
    ...
    while remaining > 0:
        ...
        if safety_count > 100:
            raise RuntimeError(
                "Too many attempts while "
                "building noise track"
            )
        ...
        if remaining <= len(noise):
            ...
            # ------------------------------------------------
            # clean 比单条 5s noise 更长。
            #
            # 此时使用整条 noise，然后继续选同类别下一条。
...
def build_valid_noise_track(
    ...
    for _ in range(
        MAX_NOISE_SAMPLE_TRIES
    ):

        noise, info = (
            build_noise_track_once(
                noise_files,
                target_len,
                rng,
            )
    ...
    raise RuntimeError(
        "Cannot find a valid noise segment "
...
def calculate_noise_scale(
    ...
    if clean_active_power <= 0:
        raise ValueError(
            "Invalid clean active power"
        )

    if noise_power < MIN_NOISE_POWER:
        raise ValueError(
            "Noise power too small"
        )
...
def validate_paths():
    ...
    if not CLEAN_DIR.exists():
        raise FileNotFoundError(
            f"CLEAN_DIR not found: "
            f"{CLEAN_DIR}"
        )

    if not NOISE_DIR.exists():
        raise FileNotFoundError(
            f"NOISE_DIR not found: "
            f"{NOISE_DIR}"
        )

    if clean_root == noise_root:
        raise ValueError(
            "CLEAN_DIR 和 NOISE_DIR "
            "不能相同。"
        )
...
def generate_kws_snr_dataset():
    ...
    with open(
        ...
        writer.writerow([
            ...
            # 目标定义使用这个
            "measured_active_snr_db",

            # 方便以后理解：
            # 如果 clean 有大量静音，
            # global_snr 通常比 active_snr 低。
        ...
        for clean_idx, clean_path in enumerate(
            clean_files,
            start=1,
        ):
            ...
            # =================================================
            # 先构造所有 noise condition。
            #
            # 这样才能计算一个全局 common gain，
            # 保证所有 noise type / SNR 下
            # clean speech 的幅度完全一致。
            # =================================================
            ...
            for (
                noise_type,
                noise_files,
            ) in noise_groups.items():

                for realization_id in range(
                    NOISE_REALIZATIONS_PER_TYPE
                ):
                    ...
                    try:
                        ...
                        print(
                            f"Noise build failed: "
                            f"{noise_type}, "
                            f"rep={realization_id}, "
                            f"{e}"
                        )
                        continue
