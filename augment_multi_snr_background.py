...
def generate_augmented_audio(clean_audio, bg_files, snr, rng=None, fixed_bg_path=None):
    """
    给纯净语音叠加指定信噪比 (SNR) 的单一背景噪声
    如果传了 fixed_bg_path，则直接使用它；否则使用 rng 随机抽取。
    """
    ...
    if bg_files:
        for _ in range(NUM_BG_NOISES):
            ...
            # 为了使同一个背景在不同 SNR 级别下混合的片段段落保持一致
            # 如果是指定了背景，我们应当重置截取起始点（默认 0，或者你可以加个 fixed_start 参数）
            # 这里统一采用直接从头截取，或者从一个固定的 rng seed
            start = 0 if fixed_bg_path else (rng.randint(0, bg_len - 1) if bg_len > 1 else 0)
            ...
            # 使用相对路径，如果想要绝对路径可以换成 str(path.resolve())
            try:
                rel_noise_path = path.relative_to(BG_NOISE_DIR)
            except ValueError:
                rel_noise_path = path.name
                
            used_bg.append(f"{rel_noise_path}@{snr}dB")
...
def main():
    ...
    with open(meta_path, "w", newline="", encoding="utf-8-sig") as f_meta:
        ...
        for idx, clean_path in enumerate(clean_files, 1):
            ...
            # 在处理这条特定语音前，提前抽取一首“命中”的固定背景噪声
            # 保证同一个 clean 音频在 4 个信噪比下，用的是完全相同的这首噪声、相同的波段起始点！
            chosen_bg = rng.choice(bg_files) if bg_files else None

            # 在规定的 SNR 列表里循环，分别输出到各自的文件下
            for current_snr in BG_SNR_LIST:
                noisy_audio, desc = generate_augmented_audio(clean, bg_files, current_snr, rng, fixed_bg_path=chosen_bg)
